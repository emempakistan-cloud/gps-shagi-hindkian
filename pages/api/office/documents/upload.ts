...
export default async function handler(
  ...
  try {
    ...
    // Check if user is admin - use supabaseAdmin since the anon client
    // has no session context server-side (would incorrectly find 0 rows)
    ...
    if (userData?.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can manage office documents' });
    }
    ...
    if (req.method === 'GET') {
      ...
      if (category && typeof category === 'string') {
        documents = await OfficeDocumentService.getDocumentsByCategory(
          category,
          supabaseAdmin
        );
      } else {
        documents = await OfficeDocumentService.getAllOfficeDocuments(supabaseAdmin);
      }
    ...
    if (req.method === 'POST') {
      ...
      const { path, fileName } = await StorageService.uploadOfficeDocument(
        category,
        fileContent,
      ...
      const document = await OfficeDocumentService.addDocumentMetadata(
        {
          category: category,
